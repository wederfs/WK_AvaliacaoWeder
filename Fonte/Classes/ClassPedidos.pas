...
procedure TClassPedidos.Limpar;
begin
  Self.NumeroPedido := 0;
...
begin
  ...
  try
    ...
    APedidos.NumeroPedido := RetornaNumeroPedido;
...
function TCadastroPedidos.RetornaNumeroPedido: Integer;
var
  FDQuery : TFDQuery;
begin
  FDQuery := TClassControle.GetInstancia.Conexao.TQuery;
  try
    FDQuery.Close;
    FDQuery.SQL.Clear;
    FDQuery.SQL.Text := 'SELECT COALESCE(MAX(NUMEROPEDIDO),1) NUMPED FROM TB_PEDIDOS';
    FDQuery.Open;
    Result := FDQuery.FieldByName('NUMPED').AsInteger;
  finally
    FreeAndNil(FDQuery);
  end;
end;
