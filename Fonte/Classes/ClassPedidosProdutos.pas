...
uses FireDAC.Comp.Client, FireDac.Stan.param, System.SysUtils,
     Datasnap.DBClient, ClassControle, ClassConexao;
...
type
  ...
  public
    ...
    procedure Carregar(ANumeroPedido : integer; ClientDataSet : TClientDataSet = Nil );
    procedure Limpar;
  end;

  TCadastroPedidosProdutos = class
    procedure CarregarPedidoProduto(APedidosprodutos : TClassPedidosProdutos; ANumeroPedido : Integer; ClientDataSet : TClientDataSet = Nil);
    ...
    Function PesquisaDescProduto(CODIGO: Integer): String;
  private
    FConexao : TConexao;
  public
    constructor Create;
...
procedure TClassPedidosProdutos.Carregar(ANumeroPedido : integer; ClientDataSet : TClientDataSet = Nil);
var
  VPedidosProdutos : TCadastroPedidosProdutos;
begin
  VPedidosProdutos := TCadastroPedidosProdutos.create;
 try
   VPedidosProdutos.CarregarPedidoProduto(self,ANumeroPedido,ClientDataSet );
 finally
   FreeAndNil(VPedidosProdutos);
 end;
end;

procedure TClassPedidosProdutos.Limpar;
begin
  Self.Codigo_Pedido_Produto := 0;
  Self.NumeroPedido          := 0;
...
procedure TCadastroPedidosProdutos.CarregarPedidoProduto(
  APedidosprodutos: TClassPedidosProdutos; ANumeroPedido: Integer; ClientDataSet : TClientDataSet = Nil);
var  
  FDQuery : TFDQuery;
begin
  FDQuery := TClassControle.GetInstancia.Conexao.TQuery;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Insert;
          FieldByName('CODIGO').Value      := FDQuery.FieldByName('Codigo_Produto').AsInteger;
          FieldByName('DESCRICAO').Value   := PesquisaDescProduto(FDQuery.FieldByName('Codigo_Produto').AsInteger);
          FieldByName('QTD').Value         := FDQuery.FieldByName('Quantidade').AsFloat;
          FieldByName('VALOR_UNIT').Value  := FDQuery.FieldByName('Valor_Unitario').AsFloat;
          FieldByName('VALOR_TOTAL').Value := FDQuery.FieldByName('Valor_Total').AsFloat;
          Post;
        end;
      end;
    end;
...
function TCadastroPedidosProdutos.PesquisaDescProduto(CODIGO: Integer): String;
var
  VQuery : TFDQuery;
begin
